   ...
   public class ChatHelper
    {
        public static void SendMsg(string name,string msg="") {
            ...
            foreach (KeyValuePair<long,Session> s in ls)
            {
                ...
                s.Value.Send(new G2C_TestHotfixMessage() {Info = $"{name}_{ls.Count}:-->{s.Key}"});
            }*/
            int count = GateSessionComponent.Instance.GetAllClientSession().Count;
            foreach (Session s in GateSessionComponent.Instance.GetAllClientSession())
            {
                if (s.IsDisposed) continue;
                s.Send(new G2C_TestHotfixMessage() { Info = $"{name}_{count}:-->{s.InstanceId}" });
            }
        }
    }
}
